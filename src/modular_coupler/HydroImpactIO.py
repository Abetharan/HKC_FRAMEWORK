...
def fileWriteFormat(tmpFileLocation, OpenFile, coord, varData, var):
    ...
    if var != "coord":
        ...
        cellCenteredCoordWithGhost = [-cellCenteredCoord[0]
                                      ] + cellCenteredCoord + [LastGhostCellCoord]
        leadingDim = 3
...
def HydroToImpact(fluidOutPath, kineticOutPath, cyclePath,  laserWaveLength, fluidNx, interpolator,
    ...
    fluid_ne = np.loadtxt(
        fluidOutPath + "/NumberDensityE_" + str(lastIndex) + ".txt")
    fluid_ni = np.loadtxt(
        fluidOutPath + "/NumberDensityI_" + str(lastIndex) + ".txt")
    fluid_Te = np.loadtxt(
        fluidOutPath + "/TemperatureE_" + str(lastIndex) + ".txt")
    fluid_las_dep = np.loadtxt(
        fluidOutPath + "/InverseBrem_" + str(lastIndex) + ".txt")
    fluid_brem = np.loadtxt(fluidOutPath + "/Brem_" + str(lastIndex) + ".txt")
    fluid_density = np.loadtxt(
    ...
    if normalised_values["log_lambda"] < 0:
        ...
        exit(1)
    # Energy normlisation
    Un = me * normalised_values['vte']**2 * \
        normalised_values['ne'] * 1e21 * 1e6
    ...
    ne_norm = fluid_ne / (1e6 * normalised_values['ne'] * 1e21)
    # 1e6 there to convert from m^-3 to cm^-3
    ni_norm = fluid_ni / (1e6 * normalised_values['ni'] * 1e21)
    Te_norm = (fluid_Te * (kb/e)) / normalised_values['Te']
    laser_norm = (fluid_las_dep * fluid_density) / \
        (Un / (normalised_values['tau_ei']))
    brem_norm = (fluid_brem * fluid_density) / \
        (Un / normalised_values['tau_ei'])
    ...
    impactNiFile = open(
        cyclePath + "/" + os.environ["RUN"] + "_ionden.xy", "w")
    ...
    impactLaserFile = open(
        cyclePath + "/" + os.environ["RUN"] + "_laserdep.xy", "w")
    impactRadFile = open(
        cyclePath + "/" + os.environ["RUN"] + "_rad_to_electron.xy", "w")
    np.savetxt(kineticOutPath + "/ReturnToHydro_xf.xy",
               kinetic_x, delimiter="\n")
    fileWriteFormat(cyclePath + "/tmpWrite.txt",
                    impactNeFile, kinetic_x, kinetic_ne, "ne")
    fileWriteFormat(cyclePath + "/tmpWrite.txt",
                    impactNiFile, kinetic_x, kinetic_ni, "ni")
    fileWriteFormat(cyclePath + "/tmpWrite.txt", impactXFile,
                    kinetic_x, kinetic_x, "coord")
    fileWriteFormat(cyclePath + "/tmpWrite.txt",
                    impactTeFile, kinetic_x, kinetic_Te, "Te")
    fileWriteFormat(cyclePath + "/tmpWrite.txt", impactLaserFile,
                    kinetic_x, kinetic_laser, "laser")
    fileWriteFormat(cyclePath + "/tmpWrite.txt", impactRadFile,
                    kinetic_x, kinetic_brem, "Rad")
    fileWriteFormat(cyclePath + "/tmpWrite.txt", impactZfile,
                    kinetic_x, kinetic_Z, "zstar")
...
def ImpactToHydro(nextStepFluidInit, previousFluidOutPath, previousKineticOutPath, normalisedValues,  gammaFactor, laserWaveLength, laserPower, fluidNx):
    ...
    for var in varList:
        ...
        varArrays = cf.load_dict(
            previousKineticOutPath, runName, var, str(time_step), iter_number=None)
        ...
        if var == "n":
            ...
            normConst = 9.11E-31 * \
                normalisedValues['vte']**3*normalisedValues['ne'] * 1e21 * 1e6
...
def CoordGenerator(xNorm, velocity, kineticNx, numberSmoothingCellsRatio, numberCellsToCheck=2):
    ...
    kinetic_finer_coord = np.linspace(
        xNorm[index_max_v - numberCellsToCheck], xNorm[index_max_v + numberCellsToCheck], numberSmoothingCells)
    kinetic_right_coord = np.linspace(
        xNorm[0], xNorm[index_max_v - numberCellsToCheck - 1], no_cells_to_left)
    kinetic_left_coord = np.linspace(
        xNorm[index_max_v + numberCellsToCheck + 1], xNorm[-1], no_cells_to_right)
    kinetic_coord = np.concatenate(
        (kinetic_right_coord, kinetic_finer_coord, kinetic_left_coord), axis=0)
...
def ImpactToHydro1(normalisedValues, nextStepFluidInit,  previousFluidInit, previousFluidOutPath, previousKineticOutPath, interpolator):
    ...
    varArrays = cf.load_dict(previousKineticOutPath,
