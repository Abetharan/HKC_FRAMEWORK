...
import hkc_framework.couple_hykict as hykict
import hkc_framework.couple_impact as impact
import hkc_framework.couple_sol_kit as solkit
...
class Coupler:
    ...
    def main(self):
        ...
        if init.yaml_file['CONTINUE']:
            #Allows Continue to work without requiring to start
            #a new restart if run fails. It will just continue
            #on its own. 
        ...
        for cycle_no in range(start_cycle, cycles, 1):
            ...
            if cycle_no >= 1:
                io_obj.initialise_all_folders = False
                io_obj.overwrite = False
