...
class Multiplier: 
    ...
    def __init__(self, fluid_yaml, start_kin):
        self.fluid_yaml = fluid_yaml 
        self.start_kin = start_kin 
        self.fluid_yaml['Switches']
        self.time_step = self.fluid_yaml['TimeParameters']['steps']
        self.time_tmax = self.fluid_yaml['TimeParameters']['t_max']
        if not self.start_kin:
            self.fluid_yaml['Switches']['CoupleMulti'] = True 
        if self.start_kin:
            self.fluid_yaml['TimeParameters']['steps'] = 0
            self.fluid_yaml['TimeParameters']['t_max'] = 0
        self.search_tolerance = 1e-9

    def setSwitches(self):
        self.fluid_yaml['Switches']['CoupleMulti'] = True 
        self.fluid_yaml['TimeParameters']['steps'] = self.time_step 
        self.fluid_yaml['TimeParameters']['t_max'] = self.time_tmax

    def setParameters(self):
        self.fluid_yaml['FixedParameters']['Preheat_StartIndex'] = self.pre_heat_start_index
        self.fluid_yaml['FixedParameters']['Preheat_LastIndex'] = self.pre_heat_last_index
        self.fluid_yaml['FixedParameters']['Frontheat_StartIndex'] = self.front_heat_start_index
        self.fluid_yaml['FixedParameters']['Frontheat_LastIndex'] = self.front_heat_last_index
    ...
    def method(self, sh_heat_flow, vfp_heat_flow, cell_wall_coord, snb_heat_flow = None, laser_dir = None,):
        """ Purpose: Find multipliers and exponentially extrapolate for pre-heat
            Returns: Multipliers
        """

        self.spitzer_harm_heat = sh_heat_flow
        self.q_snb = snb_heat_flow
        self.vfp_heat = vfp_heat_flow 
        self.cell_wall_coord = cell_wall_coord
